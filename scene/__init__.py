...
# Changes support custom scene loading and mesh-based initialization.
...
class Scene:
    ...
    def __init__(self, args : ModelParams, triangles : TriangleModel, init_opacity, set_sigma, load_iteration=None, shuffle=True, resolution_scales=[1.0], segment=False, ratio_threshold=0.75):
        ...
        _start_from_mesh = False
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval)
            print("colmap init")
            _start_from_mesh = True
        elif os.path.exists(os.path.join(args.source_path, "transforms_train.json")):
            print("Found transforms_train.json file, assuming Blender data set!")
            scene_info = sceneLoadTypeCallbacks["Blender"](args.source_path, args.white_background, args.eval)
        elif os.path.exists(os.path.join(args.source_path,"image_pose.csv")):
            print("Initalizing with SLAMesh")
            scene_info = sceneLoadTypeCallbacks["SLAMesh"](args.source_path, args.eval)
            _start_from_mesh = True
        else:
        ...
        elif _start_from_mesh:
            print("start from mesh")
            mesh_path = args.source_path + "/rec_mesh_2x.ply"
            self.triangles.create_from_mesh(mesh_path, scene_info.point_cloud, init_opacity, set_sigma)
        else:
            print("start from pcd")
