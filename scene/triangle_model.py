...
# Changes add mesh initialization and geometry-guided triangle densification.
...
class TriangleModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self.depth_error_pixel = None
        self.normal_error_pixel = None
        self.geom_based_probs = None
        self.depth_based_probs = None
        self.normal_based_probs = None
        self.depth_var_probs = None
        self.normal_var_probs = None
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, opacity : float, set_sigma : float):
        ...
        print(f"Point range: Min {_points.min()}, Max {_points.max()}")
        ################################
        ...
        self.image_size = torch.zeros((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda")
        self.importance_score = torch.zeros((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda")
        self.geom_based_probs = torch.ones((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda") * 0.05
        self.depth_based_probs= torch.zeros((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda")
        self.normal_based_probs= torch.zeros((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda")
        self.depth_var_probs= torch.zeros((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda")
        self.normal_var_probs= torch.zeros((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda")
        
    def create_from_mesh(self, mesh_path, pcd : BasicPointCloud, opacity : float, set_sigma : float):
        print(mesh_path)
        print("============== creating from mesh===================")
        # load mesh file
        mesh = trimesh.load_mesh(mesh_path)
        faces = mesh.faces
        vertex = mesh.vertices  # from enlarged mesh, mesh itself is modified by xyz init coord
        mesh_vertex = vertex

        mesh_vertex = torch.tensor(np.array(mesh_vertex)).float().cuda() # [N * 3]
        print("Mesh with ", faces.shape[0], " faces loaded")

        # create feature color
        total_colors = np.ones((mesh_vertex.shape[0], 3)) * 0.5     # intialize each vertex with gray color
        ...
        # Step 3: Sort to ignore winding order
        faces = torch.tensor(np.asarray(faces)).int().cuda()
        ...
        # finally stash on your module
        self.vertices          = nn.Parameter(mesh_vertex.requires_grad_(True))
        self._triangle_indices = faces.to(torch.int32)                         # [T,3]
        vert_weight = inverse_sigmoid(opacity * torch.ones((self.vertices.shape[0], 1), dtype=torch.float, device="cuda")) 
        self.vertex_weight = nn.Parameter(vert_weight.requires_grad_(True))
        ...
        self.importance_score = torch.zeros((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda")
        self.geom_based_probs = torch.ones((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda") * 0.05
        self.depth_based_probs = torch.ones((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda") * 0.01
        self.normal_based_probs = torch.ones((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda") * 0.01
        self.depth_var_probs = torch.ones((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda") * 0.0
        self.normal_var_probs = torch.ones((self._triangle_indices.shape[0]), dtype=torch.float, device="cuda") * 0.0
    ...
    def training_setup(self, training_args, lr_mask, lr_features, weight_lr, lr_sigma, lr_triangles_init):
        ...
        l = [
            ...
            {'params': [self._features_rest], 'lr': lr_features / 50.0, "name": "f_rest"},
        ...
        # control about vertices point, not color/opacity
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "vertices":
                if iteration < 1000:  # default: 1000
                    lr = 0
                else:
    ...
    def _prune_vertices(self, vertex_mask: torch.Tensor):
        ...
        if self._triangle_indices.numel() > 0:
            ...
            if isinstance(self.geom_based_probs, torch.Tensor) and self.geom_based_probs.numel() > 0:
                self.geom_based_probs= self.geom_based_probs[valid_tris]
            if isinstance(self.depth_based_probs, torch.Tensor) and self.depth_based_probs.numel() > 0:
                self.depth_based_probs= self.depth_based_probs[valid_tris]
            if isinstance(self.normal_based_probs, torch.Tensor) and self.normal_based_probs.numel() > 0:
                self.normal_based_probs= self.normal_based_probs[valid_tris]
            if isinstance(self.depth_var_probs, torch.Tensor) and self.depth_var_probs.numel() > 0:
                self.depth_var_probs= self.depth_var_probs[valid_tris]
            if isinstance(self.normal_var_probs, torch.Tensor) and self.normal_var_probs.numel() > 0:
                self.normal_var_probs= self.normal_var_probs[valid_tris]
    ...
    def prune_triangles(self, mask):
        ...
        self.geom_based_probs= self.geom_based_probs[mask]
        self.depth_based_probs= self.depth_based_probs[mask]
        self.normal_based_probs= self.normal_based_probs[mask]
        self.depth_var_probs= self.depth_var_probs[mask]
        self.normal_var_probs= self.normal_var_probs[mask]
    ...
    def _clone_params_fast(self, selected_indices, iteration):
        """
        Creates a 'copy' of the selected triangles.
        The new triangles are NOT connected to the mesh. 
        They get 3 new vertices each, allowing them to float freely.
        """
        selected_indices = torch.unique(selected_indices)
        selected_triangles_indices = self._triangle_indices[selected_indices]  # [S, 3]
        S = selected_triangles_indices.shape[0]

        # 1. Get original vertex data
        # shape: [S, 3, 3] -> (Triangles, Vertices per Tri, XYZ)
        v_pos = self.vertices[selected_triangles_indices] 
        
        # 2. Compute Normals to offset the new layer slightly
        # This prevents the clone from being exactly inside the parent (Z-fighting/Gradient confusion)
        edge1 = v_pos[:, 1] - v_pos[:, 0]
        edge2 = v_pos[:, 2] - v_pos[:, 0]
        normals = torch.cross(edge1, edge2, dim=1)
        normals = torch.nn.functional.normalize(normals, dim=1) # [S, 3]
        
        # Expand normals to match vertex shape [S, 3, 3]
        # We push the new layer slightly along the normal
        offset_dist = 0.01  # Small offset
        offsets = normals.unsqueeze(1).repeat(1, 3, 1) * offset_dist
        
        # Optional: Add tiny random noise to help optimization break symmetry
        noise = (torch.rand_like(v_pos) - 0.5) * 0.01
        
        new_vertices_flat = (v_pos + offsets + noise).reshape(-1, 3) # [S*3, 3]

        # 3. Create new Triangle Indices
        # Since every triangle gets 3 brand new vertices, the indices are just sequential
        # Start index for new vertices
        start_idx = self.vertices.shape[0]
        
        # Create sequence [0, 1, 2, 3, 4, 5, ...]
        seq = torch.arange(S * 3, device=self.vertices.device, dtype=torch.int32)
        new_triangles = (seq + start_idx).reshape(S, 3)

        # 4. Copy Attributes
        # We need to fetch attributes for every vertex of every selected triangle
        flat_indices = selected_triangles_indices.flatten() # [S*3]

        # Color Features
        new_features_dc = self._features_dc[flat_indices]
        new_features_rest = self._features_rest[flat_indices]

        # Opacity
        # We slightly lower the opacity of clones to encourage the optimizer to only keep them if needed
        old_weights = self.vertex_weight[flat_indices]
        opacity = self.opacity_activation(old_weights)
        new_opacity = opacity * 0.95 
        new_vertex_weight = self.inverse_opacity_activation(new_opacity)
    ...
    def add_new_gs_VAR(self, iteration, cap_max, splitt_large_triangles, probs_opacity=True):
        ...
        if num_gs <= 0:
            print("num triangle max reached")
            return 0

        # Find indexes based on proba
        areas = self.triangle_areas().squeeze()
        depth_probs = self.depth_var_probs.squeeze()
        normal_probs = self.normal_var_probs.squeeze()
        triangle_transp = self.importance_score
        img_probs = triangle_transp.squeeze()

        # set blend weights
        lambda_img_probs = 0.1  # default: 0.1
        lambda_depth_probs = 1
        lambda_normal_probs = 1

        # normalize, combine, and mask
        i_norm = img_probs / (img_probs.max() + 1e-7)
        depth_norm = depth_probs / (depth_probs.max() + 1e-7)
        depth_probs = lambda_depth_probs * depth_norm + lambda_img_probs * i_norm
        depth_probs = torch.where(areas < self.size_probs_zero, torch.zeros_like(depth_probs), depth_probs)
        depth_probs = torch.where(self.image_size < self.size_probs_zero_image_space, torch.zeros_like(depth_probs), depth_probs) # dont clone if smaller than 10
        normal_norm = normal_probs / (normal_probs.max() + 1e-7)
        normal_probs = lambda_normal_probs * normal_norm + lambda_img_probs * i_norm
        normal_probs = torch.where(areas < self.size_probs_zero, torch.zeros_like(normal_probs), normal_probs)
        normal_probs = torch.where(self.image_size < self.size_probs_zero_image_space, torch.zeros_like(normal_probs), normal_probs) # dont split if smaller than 10

        # sample based on probs
        clone_rand_idx = self._sample_alives(probs=depth_probs, num=int(num_gs))    # clone idx 
        split_rand_idx = self._sample_alives(probs=normal_probs, num=int(num_gs))  # splitt idx
        ...
        # clone by index
        print("Cloning ", clone_rand_idx.shape[0], " triangles.")
        (new_vertices, new_vertex_weight, new_features_dc, new_features_rest, new_triangles) = self._clone_params_fast(clone_rand_idx, iteration)
        self.densification_postfix(new_vertices, new_vertex_weight, new_features_dc, new_features_rest, new_triangles)

        # splitt by index
        add_idx = torch.unique(torch.cat([split_rand_idx, top_idx.to(split_rand_idx.device)]), sorted=False)
        print("Splitting ", add_idx.shape[0], " triangles.")
    ...
    def add_new_gs_default(self, iteration, cap_max, splitt_large_triangles, probs_opacity=True):
    ...
    def update_geom_based_probs(self, geom_based_probs_np):
        self.geom_based_probs = torch.tensor(geom_based_probs_np, device="cuda")

    def update_depth_based_probs(self, depth_based_probs_np):
        self.depth_based_probs = torch.tensor(depth_based_probs_np, device="cuda")

    def update_normal_based_probs(self, normal_based_probs_np):
        self.normal_based_probs = torch.tensor(normal_based_probs_np, device="cuda")

    def update_normal_var_probs(self, new_var):
        self.normal_var_probs = new_var

    def update_depth_var_probs(self, new_var):
        self.depth_var_probs = new_var 

    def enlarge_triangles(self, scale_factor=1.05):
        """
        Enlarges each triangle independently by scaling vertices away from the triangle center.
        scale_factor > 1.0: Enlarge
        scale_factor < 1.0: Shrink
        """
        # 1. Get vertex positions for every triangle [N_triangles, 3_vertices, 3_coords]
        # Using self._triangle_indices to fetch the actual 3D positions [cite: 126]
        tri_v_pos = self.vertices[self._triangle_indices] 

        # 2. Compute the barycenter (arithmetic mean) of each triangle 
        # shape: [N_triangles, 1, 3_coords]
        centers = torch.mean(tri_v_pos, dim=1, keepdim=True)

        # 3. Calculate direction vectors from center to each vertex
        # shape: [N_triangles, 3_vertices, 3_coords]
        directions = tri_v_pos - centers

        # 4. Apply the scaling factor
        new_tri_v_pos = centers + (directions * scale_factor)

        # 5. Update the global vertex buffer
        flat_indices = self._triangle_indices.flatten()
        flat_new_pos = new_tri_v_pos.reshape(-1, 3)

        self.vertices[flat_indices] = flat_new_pos
        
        print(f"Enlarged {self._triangle_indices.shape[0]} triangles by factor {scale_factor}")

    def accumulate_positional_grad(self, visibility_filter):
        """
        Accumulates the magnitude of 3D positional gradients.
        """
        # 1. Access the gradient of the 3D vertex positions
        if self.vertices.grad is None:
            return

        # 2. Initialize buffers if needed
        if not hasattr(self, 'pos_grad_accum'):
            self.pos_grad_accum = torch.zeros(self.vertices.shape[0], device="cuda")
            self.pos_denom = torch.zeros(self.vertices.shape[0], device="cuda")

        # 3. Calculate 3D gradient magnitude: sqrt(dx^2 + dy^2 + dz^2)
        grad_mag = torch.norm(self.vertices.grad, dim=-1) # [N_vertices]

        # 4. Map triangle visibility to vertex indices
        visible_triangle_indices = self._triangle_indices[visibility_filter] # [N_visible_tri, 3]
        visible_vertex_indices = visible_triangle_indices.flatten().long() # [N_visible_tri * 3]

        # 5. Accumulate magnitudes for vertices associated with visible triangles
        self.pos_grad_accum[visible_vertex_indices] += grad_mag[visible_vertex_indices]
        self.pos_denom[visible_vertex_indices] += 1

    def get_positional_grads(self):
        """Returns averaged 3D positional gradients per vertex."""
        return self.pos_grad_accum / torch.clamp(self.pos_denom, min=1.0)

    def zero_positional_grad(self):
        """Resets the accumulation buffers."""
        if hasattr(self, 'pos_grad_accum'):
            self.pos_grad_accum.zero_()
            self.pos_denom.zero_()
    
    def add_new_gs_positional(self, iteration, cap_max, positional_grad_threshold=0.00001):
        """
        Subdivides triangles based on view-space (photometric) gradients.
        Triggers subdivision in areas where the image error is high.
        """
        current_num_points = self.vertices.shape[0]
        target_num = min(cap_max, int(self.add_percentage * current_num_points))
        num_to_add = max(0, target_num - current_num_points)

        if num_to_add <= 0:
            print(f"Num max triangle reached")
            return 0

        # 1. Identify triangles with high photometric gradients
        grads = self.get_positional_grads() # [N_triangles]
        print("grads: ", grads.max(), grads.min())
        
        # 2. Select candidates where gradient exceeds the threshold
        selected_mask = grads > positional_grad_threshold
        candidate_indices = torch.where(selected_mask)[0]

        # 3. Limit to the number of points we are allowed to add
        if candidate_indices.shape[0] > num_to_add:
            _, top_k_idx = torch.topk(grads[candidate_indices], num_to_add)
            add_idx = candidate_indices[top_k_idx]
        else:
            add_idx = candidate_indices

        if add_idx.shape[0] == 0:
            print(f"nothing to add by threshold")
            return 0

        # 4. Use the existing fast subdivision logic to refine these areas
        (new_vertices, new_vertex_weight, new_features_dc, new_features_rest, new_triangles) = self._update_params_fast(add_idx, iteration)

        # 5. Integrate new geometry
        self.densification_postfix(new_vertices, new_vertex_weight, new_features_dc, new_features_rest, new_triangles)

        # 6. Remove the coarse 'parent' triangles to maintain the mesh structure
        mask = torch.ones(self._triangle_indices.shape[0], dtype=torch.bool, device=self._triangle_indices.device)
        mask[add_idx] = False
        self.prune_triangles(mask)

        print(f"Positional densification: added {add_idx.shape[0]} triangles.")

    def reset_opacity(self):
        print("reset opacity")
        self.vertex_weight = torch.ones_like(self.vertex_weight) * 0.2