...
# Changes expose depth distortion and per-pixel triangle contribution outputs.
...
def render(viewpoint_camera, pc : TriangleModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    rendered_image, radii, scaling, allmap, max_blending, triangle_indices_per_pixel, triangle_weights_per_pixel, num_contributors_per_pixel  = rasterizer(
        vertices=vertices,
    ...
    # print("idx: ", triangle_indices_per_pixel[:,1,1])
    # print("weight: ", triangle_weights_per_pixel[:,1,1])
    # print("weight_sum: ", triangle_weights_per_pixel.sum(dim=0))
    # print("num: ", num_contributors_per_pixel.shape)
    ...
    rets =  {"render": rendered_image,
            ...
            "idx_per_pixel": triangle_indices_per_pixel,
            }
    ...
    # visualize alpha
    # alpha_np = render_alpha.detach().cpu().numpy()
    # alpha_2d = np.squeeze(alpha_np)
    # alpha_uint8 = (alpha_2d * 255.0).clip(0, 255).astype(np.uint8)
    # heatmap_bgr = cv2.applyColorMap(alpha_uint8, cv2.COLORMAP_INFERNO)
    # cv2.imshow("Gaussian Alpha Heatmap", heatmap_bgr)
    # cv2.waitKey(1)
    ...
    # render_normal  = render_normal / torch.clamp(render_alpha, min=1e-8).detach()   # 
    # norm = torch.norm(render_normal, dim=0, keepdim=True)
    # norm = torch.clamp(norm, min=1e-8)
    # render_normal = render_normal / norm
    ...
    # rendered depth distortion
    distortion = allmap[7:8]
