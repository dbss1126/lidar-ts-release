...
// Modified by Byoungkwon Yoon and contributors, 2025.
// Changes allocate and return depth-distortion and triangle contribution buffers.
	...
	const bool debug)
{
  ...
  torch::Tensor triangle_indices_per_pixel = torch::full({MAX_CONTRIBUTORS_PER_PIXEL, H, W}, 0, int_opts);
  torch::Tensor triangle_weights_per_pixel = torch::full({MAX_CONTRIBUTORS_PER_PIXEL, H, W}, 0.0, float_opts);
  torch::Tensor num_contributors_per_pixel = torch::full({H, W}, 0, int_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		triangle_indices_per_pixel.contiguous().data_ptr<int>(),
		triangle_weights_per_pixel.contiguous().data_ptr<float>(),
		num_contributors_per_pixel.contiguous().data_ptr<int>(),
		radii.contiguous().data_ptr<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_others, radii, geomBuffer, binningBuffer, imgBuffer, scaling, max_blending, triangle_indices_per_pixel, triangle_weights_per_pixel, num_contributors_per_pixel);
}
