...
// Changes add per-pixel triangle contribution tracking and depth-distortion output.
 ...
 #define MAX_CONTRIBUTORS_PER_PIXEL 32  // Adjust based on your needs
    ...
    const glm::vec3* cam_pos)
{
 ...
 // Modified render kernel with contribution tracking
template <uint32_t CHANNELS>
__global__ void __launch_bounds__(BLOCK_X * BLOCK_Y)
renderCUDA_with_tracking(
	const uint2* __restrict__ ranges,
	const uint32_t* __restrict__ point_list,
	int W, int H,
	const float2* __restrict__ normals,
	...
	float* __restrict__ out_others,
	float* __restrict__ max_blending,
	// NEW: Triangle contribution tracking outputs
	int* __restrict__ triangle_indices_per_pixel,      // (H, W, MAX_CONTRIBUTORS_PER_PIXEL)
	float* __restrict__ triangle_weights_per_pixel,    // (H, W, MAX_CONTRIBUTORS_PER_PIXEL)
	int* __restrict__ num_contributors_per_pixel)      // (H, W)
{
	// Identify current tile and associated min/max pixel range.
	auto block = cg::this_thread_block();
	uint32_t horizontal_blocks = (W + BLOCK_X - 1) / BLOCK_X;
	...
	// NEW: Track triangle contributions for this pixel
	int local_triangle_ids[MAX_CONTRIBUTORS_PER_PIXEL];
	float local_triangle_weights[MAX_CONTRIBUTORS_PER_PIXEL];
	int local_num_contributors = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// NEW: Store triangle contribution
			if (local_num_contributors < MAX_CONTRIBUTORS_PER_PIXEL) {
				local_triangle_ids[local_num_contributors] = j_id;
				local_triangle_weights[local_num_contributors] = blending_weight;
				local_num_contributors++;
			}
	...
	// All threads that treat valid pixel write out their final rendering data
	if (inside)
	{	
		out_others[pix_id + 0 * H * W] = last_contributor;
		final_T[pix_id] = T;
		...
		out_others[pix_id + DEPTH_OFFSET * H * W] = D;
		// out_others[pix_id + ALPHA_OFFSET * H * W] = 1 - T;
		out_others[pix_id + ALPHA_OFFSET * H * W] = Weight;
		for (int ch=0; ch<3; ch++) out_others[pix_id + (NORMAL_OFFSET+ch) * H * W] = N[ch];
		out_others[pix_id + MIDDEPTH_OFFSET * H * W] = median_depth;
		out_others[pix_id + DISTORTION_OFFSET * H * W] = pixel_influence;
		out_others[pix_id + (DISTORTION_OFFSET+1) * H * W] = distortion;

		// NEW: Write triangle contribution data
		num_contributors_per_pixel[pix_id] = local_num_contributors;
		
		// Write triangle indices and weights
		for (int k = 0; k < MAX_CONTRIBUTORS_PER_PIXEL; k++) {
			int output_idx = pix_id * MAX_CONTRIBUTORS_PER_PIXEL + k;
			if (k < local_num_contributors) {
				triangle_indices_per_pixel[output_idx] = local_triangle_ids[k];
				triangle_weights_per_pixel[output_idx] = local_triangle_weights[k];
			} else {
				triangle_indices_per_pixel[output_idx] = -1;  // Invalid marker
				triangle_weights_per_pixel[output_idx] = 0.0f;
			}
		}
	}
}

// Wrapper function to call the modified kernel
void FORWARD::render_with_tracking(
	const dim3 grid, dim3 block,
	const uint2* ranges,
	const uint32_t* point_list,
	int W, int H,
	...
	float* max_blending,
	// NEW parameters
	int* triangle_indices_per_pixel,
	float* triangle_weights_per_pixel,
	int* num_contributors_per_pixel)
{
	renderCUDA_with_tracking<NUM_CHANNELS><<<grid, block>>>(
		ranges,
		point_list,
		W, H,
		normals,
		...
		out_others,
		max_blending,
		triangle_indices_per_pixel,
		triangle_weights_per_pixel,
		num_contributors_per_pixel
	);
}